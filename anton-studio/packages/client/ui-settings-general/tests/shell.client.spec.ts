...
describe('ui-settings apply', () => {
  ...
  it('projects the section ledger into ordered nav rows with option defaults', async () => {
    ...
    // This package registers General and API Providers itself; every other
    // section arrives from a feature registrant.
    const GENERAL = { id: 'general', order: 0, label: 'general.nav' }
    const API_PROVIDERS = { id: 'api-providers', order: 1, label: 'API Providers' }
    expect(sections.getSnapshot()).toEqual([GENERAL, API_PROVIDERS])
