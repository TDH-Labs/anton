...
import { OpsNav } from '../src/client/OpsNav.tsx'
import { SidebarRoot } from '@deepseek-ai/dsh-client-ui-sidebar/src/client/SidebarRoot.tsx'
  ...
  function mountBadge(fetchJson: unknown[] | 'error') {
    ...
    render(
      <SidebarRoot
        ...
        renderSlot={((key: string, owner: { wide: boolean }) => (
          // Mirrors index.ts's real `sidebar.nav` registration — SidebarRoot
          // itself owns no store; OpsNav is the actual occupant that reads
          // the approvals count and the active screen.
          key === 'sidebar.nav'
            ? <OpsNav useStore={bindStoreHook(store)} actions={store.actions} wide={owner.wide} />
            : <div data-testid="seat" />
