...
/** Matches GET /api/agent/worklog (README, Data Contracts). */
type Worklog = { ongoing: { text: string; meta: string; pct: string | null }[]; done: { text: string; meta: string }[] }
          ...
          <div>
            ...
            <div style={{ borderTop: LN }}>
              ...
              {systems.map((s) => {
                ...
                return (
                  <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '26px minmax(0,1.5fr) minmax(0,1.6fr) 92px 118px 20px', alignItems: 'center', gap: 14, padding: '13px 8px', borderBottom: LN, cursor: 'pointer' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 26 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  {worklog.done.map((w, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'baseline', gap: 9, fontSize: 13, color: 'var(--dsw-alias-label-secondary)' }}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--dsw-alias-state-success-primary)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flex: 'none' }}><path d="M20 6 9 17l-5-5" /></svg>
                      <span style={{ flex: 1 }}>{w.text}</span>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 11 }}>
                ...
                <span style={{ fontSize: 12, cursor: 'pointer', color: 'var(--dsw-alias-accent-strong)', borderBottom: '1px solid currentColor' }}>Open the inbox</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                {approvals.map(a => (
                  <div key={a.id} className={bp.blueprint} style={{ padding: '13px 15px', cursor: 'pointer' }}>
                    <span className={`${bp.corner} ${bp.cornerTl}`} />
                    <span className={`${bp.corner} ${bp.cornerTr}`} />
                    <span className={`${bp.corner} ${bp.cornerBl}`} />
                    <span className={`${bp.corner} ${bp.cornerBr}`} />
