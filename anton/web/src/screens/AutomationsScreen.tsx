...
// "Describe it" and "Upload a doc" POST /api/automations/draft (the
// configured model drafts a JSON workflow that is strictly validated
// server-side) -- a returned draft is only ever a reviewable card, saving it
// goes through PUT /api/automations/:id as awaiting_approval, never running.
// "Draw it" is deliberately not a competing in-app canvas: real workflow
// construction (real connectors, real node-by-node testing) belongs in n8n,
// not a hand-built imitation of it. It opens the operator's own n8n editor
// (config.yaml n8n.base_url / ANTON_N8N_BASE_URL, see dashboard.py's
// /api/n8n/config) in a new tab.
...
const MAKE_WAYS: MakeWay[] = [
  ...
  { icon: 'M3 3h7v7H3zM14 3h7v7h-7zM14 14h7v7h-7zM3 14h7v7H3z', title: 'Draw it', desc: 'Opens your n8n editor — real connectors, real testing' },
...
/** Automations (README §3): live automation list, plus the node editor (§4)
 * opened from a row or a card to review/approve an already-drafted
 * automation -- never to build one from scratch (see "Draw it" above). */
export function AutomationsScreen() {
  const { data, loading, error, refetch } = useOpsApi<Automation[]>('/api/initiatives')
  const { data: n8nConfig } = useOpsApi<{ base_url: string | null }>('/api/n8n/config')
  const [editingId, setEditingId] = useState<string | null>(null)
  const [editingName, setEditingName] = useState<string>('')
  const [n8nNotice, setN8nNotice] = useState(false)
  ...
  if (editingId !== null) {
    ...
    const base = n8nConfig?.base_url ?? null
    return (
      <div style={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column', overflowY: 'auto', padding: '22px 26px 30px', background: 'var(--dsw-alias-bg-base)' }}>
        <div className={bp.kicker}>EDIT IN n8n</div>
        <div className={bp.screenTitle} style={{ fontSize: 20, marginBottom: 10 }}>{editing?.name ?? editingName}</div>
        <div style={{ fontSize: 13.5, lineHeight: 1.5, color: 'var(--dsw-alias-label-secondary)', maxWidth: 620, marginBottom: 16 }}>
          Anton drafts the steps; n8n is where they are wired to real services and tested
          node by node. Editing happens there so you get its actual connector library
          rather than a lookalike canvas that can only draw the shape of the work.
        </div>
        {editing !== undefined && editing.nodes.length > 0 && (
          <div style={{ border: LN, padding: 14, marginBottom: 16, maxWidth: 620 }}>
            <div className={bp.kicker} style={{ marginBottom: 8 }}>THE STEPS ANTON DRAFTED</div>
            {editing.nodes.map(n => (
              <div key={n.id} className={bp.mono} style={{ fontSize: 12.5, padding: '3px 0' }}>
                {n.kind === 'human' ? '◆' : '●'}  {n.text}
              </div>
            ))}
          </div>
          ...
          {base !== null
            ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                style={{ padding: '7px 16px', background: 'var(--dsw-alias-state-business-primary)', color: 'var(--dsw-alias-bg-base)', fontSize: 13, textDecoration: 'none' }}
              >Open n8n</a>
            )
            : (
              <span style={{ fontSize: 13, color: 'var(--dsw-alias-label-secondary)' }}>
                No n8n instance connected yet — add one in Settings, then this opens straight into it.
              </span>
            )}
          <button
            onClick={() => { setEditingId(null); refetch() }}
            style={{ padding: '7px 16px', background: 'transparent', color: 'var(--dsw-alias-label-primary)', border: LN, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}
          >Back</button>
        </div>
      </div>
    )
    ...
    <div style={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column', background: 'var(--dsw-alias-bg-base)' }}>
      <div style={{ flex: 'none', display: 'flex', alignItems: 'flex-end', gap: 16, padding: '18px 26px 14px', borderBottom: LN }}>
        <div style={{ minWidth: 0 }}>
          <div className={bp.kicker}>{automations.filter(a => a.state === 'awaiting_approval').length} DRAFTED · {automations.filter(a => a.author === 'human').length} BUILT BY YOU</div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
        <div style={{ padding: '22px 26px 30px', minWidth: 900 }}>
            ...
            {MAKE_WAYS.map((w, i) => (
              <div
                ...
                onClick={() => {
                  ...
                  else if (n8nConfig?.base_url) { window.open(n8nConfig.base_url, '_blank', 'noopener') }
                  else { setN8nNotice(true) }
                }}
          ...
          {n8nNotice && (
            <div style={{ fontSize: 13, color: 'var(--dsw-alias-label-secondary)', marginBottom: 24 }}>
              No n8n instance connected yet — set it up in Settings, then "Draw it" opens straight into your editor.{' '}
              <span onClick={() => { setN8nNotice(false) }} style={{ color: 'var(--dsw-alias-state-business-primary)', cursor: 'pointer', borderBottom: '1px solid currentColor' }}>Dismiss</span>
            </div>
          )}
